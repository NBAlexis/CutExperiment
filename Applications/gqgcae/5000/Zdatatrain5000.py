...
dataoriginal = np.loadtxt('G:\科研\检测异常度\data\Features\SM-5000.csv', delimiter=',')
print(dataoriginal)
...
num_epoch = 500
batch_size = 32
checkpointer = ModelCheckpoint(filepath="autoencoder10.h5",
                               verbose=0,
                               save_best_only=True)
autoencoder10 = autoencoder.fit(datatrain, datatrain,
                               epochs=num_epoch,
...
plt.plot(autoencoder10["loss"], c='dodgerblue', lw=3)
plt.plot(autoencoder10["val_loss"], c='coral', lw=3)
plt.title('model loss')
plt.ylabel('mse');
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper right')
plt.show()
print(autoencoder10["loss"])
print(autoencoder10["val_loss"])

#输出数据到CSV文件
list1=autoencoder10["loss"]
list2=autoencoder10["val_loss"]
list=[]
list.append(list1)
list.append(list2)
print(list)
test=pd.DataFrame(data=list)#将数据放进表格
test.to_csv('10TeVautoencoder1loss.csv') #数据存入csv,存储位置及文件名称
