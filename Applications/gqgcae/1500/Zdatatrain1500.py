...
# 取数据 X_train 40万；X_validation 10万；剩下为test
dataoriginal = np.loadtxt('G:\科研\检测异常度\data\Features\SM-1500.csv', delimiter=',')
print(dataoriginal)
...
checkpointer = ModelCheckpoint(filepath="autoencoder3.h5",
                               verbose=0,
                               save_best_only=True)
autoencoder3 = autoencoder.fit(datatrain, datatrain,
                               epochs=num_epoch,
...
plt.plot(autoencoder3["loss"], c='dodgerblue', lw=3)
plt.plot(autoencoder3["val_loss"], c='coral', lw=3)
plt.title('model loss')
plt.ylabel('mse');
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper right')
plt.show()
print(autoencoder3["loss"])
print(autoencoder3["val_loss"])
...
test.to_csv('3TeVautoencoder1loss.csv') #数据存入csv,存储位置及文件名称
