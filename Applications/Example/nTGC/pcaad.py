...
ax.hist(smDistance, label="SM", histtype="step", range=[minDistance, maxDistance], bins=10, density=True)
ax.hist(npDistance, label="NP", histtype="step", range=[minDistance, maxDistance], bins=10, density=True)
ax.legend()
ax.set_xlabel('d', fontsize=20)
ax.set_ylabel('1/N dN/{0:.3f}'.format((maxDistance - minDistance) / 10), fontsize=20)
plt.show()
