...
from Applications.qkmeans.jllvv.jllvvUsefullFunctions import findJetAndTwoLeptonsV3
...
for FILETYPEIDX in range(22):
    ...
    filtered = findJetAndTwoLeptonsV3(lhcoevent, missingcut, jetcut, leptoncut, 6)
    ...
    for i in range(len(loadfiles[FILETYPEIDX])):
        if 0 != i:
            ...
            if os.path.exists(sbanner):
                cs = cs / totale0
            newfiltered = findJetAndTwoLeptonsV3(lhcoevent, missingcut, jetcut, leptoncut, 6)
            if 0 == len(filtered):
    ...
    np.savetxt("featurecsvv3/{}.csv".format(savefiles[FILETYPEIDX]), filtered, delimiter=',')
    print("featurecsvv3/{}.csv saved".format(savefiles[FILETYPEIDX]))
